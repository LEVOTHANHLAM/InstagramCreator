...
using InstagramCreator.ApiQnibot;
using InstagramCreator.Entities;
using InstagramCreator.FiveSimApi;
using InstagramCreator.Forms;
using InstagramCreator.Froms;
using InstagramCreator.GetSmsCode;
using InstagramCreator.Helper;
using InstagramCreator.Instagram;
using InstagramCreator.Models;
using InstagramCreator.MuiltiTask;
using InstagramCreator.OtpServices;
using InstagramCreator.OtpServices.Codesim;
using InstagramCreator.OtpServices.CountryHelper;
using InstagramCreator.Repositories;
using InstagramCreator.Sms365Api;
...
namespace InstagramCreator
{
    public partial class fMain : Form
    {
        ...
        private readonly IInstagramController _instagramController;
        ...
        public fMain(IAccountRepository accountRepository, IInstagramController instagramController)
        {
        ...
        private async void btnStart_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (rbRegWithPhone.Checked == true)
                {
                    var result = CBoxOtpService.SelectedItem.ToString();
                    if (result == "Sim5.net" || result == "Sms365 .org")
                    {
                        if (string.IsNullOrEmpty(txtOperator.Text))
                        {
                            MessageCommon.ShowMessageBox("please enter the operator of OTP site you selected!", 2);
                            btnStart.Enabled = true;
                            btnStart.BackColor = Color.Lime;
                            return;
                        }
                ...
                if (radioRadomFullNameVN.Checked)
                {
                    _listFirtname.Clear();
                    _listLastname.Clear();
                    _listFirtname.AddRange(FileHelper.FirtnamesVN);
                    _listLastname.AddRange(FileHelper.LastnamesVN);
                }
                else if (radioRadomFullNameUs.Checked == true)
                {
                    _listFirtname.Clear();
                    _listLastname.Clear();
                    _listFirtname.AddRange(FileHelper.FirtnamesUS);
                    _listLastname.AddRange(FileHelper.LastnamesUS);
                }
                //-----------------------------------------------//
                ...
                for (int i = 0; i < numberAccount.Value; i++)
                {
                    ...
                    profile.Username = _listUsername[random.Next(0, _listUsername.Count)] + Helpers.GenerateRandomString("abcdefghijklmnopqrstuvwxyz0123456789", random.Next(5, 10));
                    ...
                    if (device.Status == "Connect")
                    {
                        ...
                        if (device.Status != "reboot")
                        {
                            ...
                            if (checkLayout == 1)
                            {
                                ...
                                if (ADBClientController.FindElementIsExistOrClickByClass(device.Data, device.AdbClient, "Sign up with email", "android.view.View", 20, false) || ADBClientController.FindElementIsExistOrClickByClass(device.Data, device.AdbClient, "Sign up with mobile number", "android.view.View", 30, true))
                                {
                                    layout = true;
                                                    ...
                                                    else if (rbRegWithPhone.Checked)
                                                    {
                                                        ...
                                                        while (GetPhone > 0)
                                                        {
                                                            switch (service)
                                                            {
                                                                case "Sim5.net":
                                                                    {
                                                                        var httpResult = await FiveSimHttpHelper.BuyPhoneNumber(GlobalModels.Contry, txtApikey.Text.Trim(), txtOperator.Text);
                                                                        if (httpResult != null)
                                                                        {
                                                                ...
                                                                case "Sms365 .org":
                                                                    {
                                                                        var getBuyPhone = await Sms365HttpHelper.BuyPhoneNumber(txtApikey.Text.Trim(), GlobalModels.Contry, txtOperator.Text);
                                                                        if (getBuyPhone != null)
                                                                        {
                                                                ...
                                                                case "Codesim.net":
                                                                    {
                                                                        string appId = "9&network_id=1";
                                                                        if (rbMobi.Checked)
                                                                        {
                                                                            appId = "9&network_id=2";
                                                                        }
                                                                        else if (rbITelecom.Checked)
                                                                        {
                                                                            appId = "9&network_id=6";
                                                                        }
                                                                        else if (rbVina.Checked)
                                                                        {
                                                                            appId = "9&network_id=4";
                                                                        }
                                                                        else if (rbVNMB.Checked)
                                                                        {
                                                                            appId = "9&network_id=5";
                                                                        }

                                                                        var getBuyPhone = await CodesimHttpHelper.BuyPhoneNumber(txtApikey.Text.Trim(), appId);
                                                                        if (getBuyPhone != null && getBuyPhone.Status == 200)
                                                                        {
                                                                            phonenumber = getBuyPhone.Data.Phone;
                                                                            idPhone = getBuyPhone.Data.OtpId.ToString();
                                                                            GetPhone = 0;
                                                                            break;
                                                                        }
                                                                        else if (getBuyPhone == null)
                                                                        ...
                                                                        {
                                                                            writeLog(device, getBuyPhone.Data.Message, null, device.view.StatusLabel, 1);
                                                                        }
                                                                        GetPhone--;
                                                                        continue;
                                                                    }
                                                        ...
                                                        {
                                                            ADBClientController.FindElementIsExistOrClickByClass(device.Data, device.AdbClient, "Create new account", "android.view.View", 10, true);
                                                            if (!string.IsNullOrEmpty(idPhone) && !string.IsNullOrEmpty(phonenumber))
                                                            {
                                                                ...
                                                                while (GetPhone > 0)
                                                                {
                                                                    ...
                                                                    switch (service)
                                                                    {
                                                                        ...
                                                                        case "Codesim.net":
                                                                            {
                                                                                var getOtp = await CodesimHttpHelper.GetOtp(txtApikey.Text.Trim(), idPhone);
                                                                                if (getOtp != null && getOtp.Status == 200)
                                                                                {
                                                                                    if (!string.IsNullOrEmpty(getOtp.Data.Code))
                                                                                    {
                                                                                        if (getOtp.Data.Code.Length == 6)
                                                                                        {
                                                                                            code = getOtp.Data.Code;
                                                                                        }
                                                                                        else
                                                                                        {
                                                                                            ...
                                                                                            Match match = Regex.Match(getOtp.Data.Code, pattern);
                                                                                            if (match.Success)
                                                                                            {
                                                                                                code = match.Value.ToString().Replace(" ", "");
                                                                                            }
                                                                                        }
                                                                                    }
                                                                                    Log.Information(code);
                                                                                    writeLog(device, getOtp.Data.Content.ToString(), null, device.view.StatusLabel, 2);
                                                                                    GetPhone = 0;
                                                                                    break;
                                                                                }
                                                                                else if (getOtp == null)
                                                                                ...
                                                                                {
                                                                                    writeLog(device, getOtp.Data.Message, null, device.view.StatusLabel, 1);
                                                                                    if (getOtp.Data.Message.Contains("timeout"))
                                                                                    {
                                                                                        GetPhone = 0;
                                                                                        break;
                                                                                    }
                                ...
                                else
                                {
                                    if (rbRegWithEmail.Checked)
                                    {
                                        ...
                                        if (ADBClientController.FindElementIsExistOrClickByClass(device.Data, device.AdbClient, "Sign up with email", "android.widget.Button", 30, true))
                                        {
                                            if (_instagramController.ImportEmailOrPhoneInstagram(device.Data, device.AdbClient, selectAccount.Email))
                                            {
                                                ...
                                                {
                                                    writeLog(device, $"import {codeEmail}", null, device.view.StatusLabel);
                                                    if (ADBClientController.InputElement(device.Data, device.AdbClient, "class='android.widget.EditText'", codeEmail, 30))
                                                    {
                                                        if (ADBClientController.FindElementIsExistOrClickByClass(device.Data, device.AdbClient, "Next", "android.widget.Button", 30, true))
                                                        {
                                                            device.Status = "imporCode";
                                                        }
                                                    }
                                                }
                                            }
                                    ...
                                    else if (rbRegWithPhone.Checked)
                                    {
                                        ...
                                        if (ADBClientController.FindElementIsExistOrClickByClass(device.Data, device.AdbClient, "Next", "android.view.View", 30, true))
                                        {
                                            ...
                                            if (!string.IsNullOrEmpty(idPhone) && !string.IsNullOrEmpty(phonenumber))
                                            {
                                                ...
                                                {
                                                    writeLog(device, $"import {code}", null, device.view.StatusLabel);
                                                    if (ADBClientController.InputElement(device.Data, device.AdbClient, "class='android.widget.EditText'", code, 30))
                                                    {
                                                        if (ADBClientController.FindElementIsExistOrClickByClass(device.Data, device.AdbClient, "Next", "android.widget.Button", 30, true))
                                                        {
                                                            device.Status = "imporCode";
                                    ...
                                    if (device.Status == "imporCode")
                                    {
                                        if (ADBClientController.FindElementIsExistOrClickByClass(device.Data, device.AdbClient, "Create a password", "android.view.View", 15, false))
                                        {
                                            writeLog(device, $"Import Password", null, device.view.StatusLabel);
                                            if (_instagramController.ImportPasswrodInstagram(device.Data, device.AdbClient, selectAccount.Password))
                                            {
                                                writeLog(device, $"Import Date of birth", null, device.view.StatusLabel);
                                                Random random = new Random();
                                                ADBHelper.SwipeByPercent(device.Id, 26.2, 41.1, 26.8, 57.9, 300, random.Next(10, 17));// thang
                                                ADBHelper.SwipeByPercent(device.Id, 48.5, 41.1, 48.5, 57.0, 300, random.Next(10, 17));// ngay
                                                ADBHelper.SwipeByPercent(device.Id, 71.7, 41.1, 71.1, 56.7, 300, random.Next(10, 15));// nam
                                                if (ADBClientController.FindElementIsExistOrClickByClass(device.Data, device.AdbClient, "SET", "android.widget.Button", 30, true))
                                                {
                                                    if (ADBClientController.FindElementIsExistOrClickByClass(device.Data, device.AdbClient, "Next", "android.widget.Button", 30, true))
                                                    {
                                                        if (ADBClientController.FindElementIsExistOrClickByClass(device.Data, device.AdbClient, "What's your name?", "android.view.View", 30, false))
                                                        {
                                                            if (ADBClientController.ClearTextElement(device.Data, device.AdbClient, "class='android.widget.EditText'", 30))
                                                            {
                                                                writeLog(device, $"Import Fullname", null, device.view.StatusLabel);
                                                                string fullname = $"{selectAccount.Firstname} {selectAccount.Lastname}";
                                                                ADBHelper.InputTextWithADBKeyboard(device.Id, fullname);
                                                                if (ADBClientController.FindElementIsExistOrClickByClass(device.Data, device.AdbClient, "Next", "android.widget.Button", 30, true))
                                                                {
                                                                    ADBHelper.Delay(5, 7);
                                                                    if (ADBClientController.InputElement(device.Data, device.AdbClient, "class='android.widget.EditText'", selectAccount.Username, 30))
                                                                    {
                                                                        writeLog(device, $"Import UserName", null, device.view.StatusLabel);
                                                                        ADBHelper.Delay(5, 7);
                                                                        if (ADBClientController.FindElementIsExistOrClickByClass(device.Data, device.AdbClient, "Next", "android.widget.Button", 30, true))
                                                                        {
                                                                            if (ADBClientController.FindElementIsExistOrClickByClass(device.Data, device.AdbClient, "I agree", "android.widget.Button", 60, true))
                                                                            {
                                                                                if (ADBClientController.FindElementIsExistOrClickByClass(device.Data, device.AdbClient, "Add a profile picture", "android.view.View", 120, false))
                                                                                {
                                                                                    device.Status = "Done";
                                                                                }
                                                                                else
                                ...
                                if (device.Status == "Done")
                                {
                                    ...
                                    // account.Backup = Path.Combine(Environment.CurrentDirectory, $"\\{userBackup}");
                                    // _accountRepository.UpdateBackupByIdAccount(account.Id, Path.Combine(EnBackupDatavironment.CurrentDirectory, $"BackupData\\{userBackup}"));
        ...
        private void CBoxOtpService_TextChanged(object sender, EventArgs e)
        {
            ...
            else if (CBoxOtpService.Text == "Yuenanka.com" || CBoxOtpService.Text == "Codesim.net")
            {
                CBServiceCode.Visible = false;
                txtOperator.Visible = false;
        ...
        {
            if (CBoxOtpService.Text == Services.WebSim5)
            {
                var line = CountryCodeHelper.GetCountry(CBServiceCode.Text.Trim()).Sim5;
            ...
            {
                var line = CountryCodeHelper.GetCountry(CBServiceCode.Text.Trim()).Sms365;
                if (!string.IsNullOrEmpty(line))
                {
                    GlobalModels.Contry = line;
                }
        ...
        {
            var contries = CountryCodeHelper.GellAllContries();
            if (contries != null && contries.Count > 0)
            {
                CBServiceCode.Items.Clear();
                foreach (var c in contries)
                {
                    CBServiceCode.Items.Add(c.Country);
                }
                var t = jsonHelper.GetValuesFromInputString("OtpCountry");
                if (!string.IsNullOrEmpty(t))
                {
                    CBServiceCode.SelectedItem = t;
                }
